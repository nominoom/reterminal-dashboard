...
function trace(fn, options = {
  ...
  const traced = function(...args) {
    ...
    const shouldTrace = (process.env.GLOBAL_CONFIG_TRACE_VERBOSE ?? process.env.EDGE_CONFIG_TRACE_VERBOSE) === "true" || options.isVerboseTrace === false;
...
function createCreateClient({
  ...
  return trace(
    function createClient2(connectionString, options = {
      ...
      const shouldUseDevelopmentCache = !options.disableDevelopmentCache && process.env.NODE_ENV === "development" && (process.env.GLOBAL_CONFIG_DISABLE_DEVELOPMENT_SWR ?? process.env.EDGE_CONFIG_DISABLE_DEVELOPMENT_SWR) !== "1";
      const api = {
        get: trace(

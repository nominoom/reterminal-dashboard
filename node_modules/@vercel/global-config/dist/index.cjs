...
var name = "@vercel/global-config";
var version = "1.5.1";

// src/utils/tracing.ts
var edgeConfigTraceSymbol = /* @__PURE__ */ Symbol.for("@vercel/global-config:global-trace");
...
function trace(fn, options = {
  ...
  const traced = function(...args) {
    ...
    const shouldTrace = (_nullishCoalesce(process.env.GLOBAL_CONFIG_TRACE_VERBOSE, () => ( process.env.EDGE_CONFIG_TRACE_VERBOSE))) === "true" || options.isVerboseTrace === false;
...
// src/utils/index.ts
var ERRORS = {
  UNAUTHORIZED: "@vercel/global-config: Unauthorized",
  EDGE_CONFIG_NOT_FOUND: "@vercel/global-config: Global Config not found"
};
var UnexpectedNetworkError = class extends Error {
  constructor(res) {
    super(
      `@vercel/global-config: Unexpected error due to response with status code ${res.status}`
    );
  }
};
...
function assertIsKey(key) {
  if (typeof key !== "string") {
    throw new Error("@vercel/global-config: Expected key to be a string");
  }
}
...
function assertIsKeys(keys) {
  if (!Array.isArray(keys) || keys.some((key) => typeof key !== "string")) {
    throw new Error(
      "@vercel/global-config: Expected keys to be an array of string"
    );
  }
}
var clone = trace(
...
function parseVercelConnectionStringFromUrl(text) {
  try {
    ...
    if (url.host !== "global-config.vercel.com" && url.host !== "edge-config.vercel.com")
      return null;
...
function createCreateClient({
  ...
  return trace(
    function createClient2(connectionString, options = {
      ...
      if (!connectionString)
        throw new Error("@vercel/global-config: No connection string provided");
      const connection = parseConnectionString(connectionString);
      if (!connection)
        throw new Error(
          "@vercel/global-config: Invalid connection string provided"
        );
      ...
      const shouldUseDevelopmentCache = !options.disableDevelopmentCache && process.env.NODE_ENV === "development" && (_nullishCoalesce(process.env.GLOBAL_CONFIG_DISABLE_DEVELOPMENT_SWR, () => ( process.env.EDGE_CONFIG_DISABLE_DEVELOPMENT_SWR))) !== "1";
      const api = {
        get: trace(
...
function init() {
  if (!defaultEdgeConfigClient) {
    defaultEdgeConfigClient = createClient(
      _nullishCoalesce(process.env.GLOBAL_CONFIG, () => ( process.env.EDGE_CONFIG))
    );
  }
}
var get = (...args) => {
